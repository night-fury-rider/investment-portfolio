...
import { updateLanguage, updateCurrency, updateLocale } from './uv_settings.actions';
...
const Settings = () => {
  ...
  const changeCurrency = (event: any)=> {
    dispatch(updateCurrency(settingsData.currency.items[event.target.selectedIndex]));
  }
        ...
        <Form className="uv-background-color-faded">
          <fieldset >
            ...
            <Form.Group as={Row}>
              ...
              <Col md={{offset: 3, span: 3}} xs={{offset: 2, span: 5}}>
                <Form.Control as="select" value={currency.title} onChange={(event)=>{changeCurrency(event)}}>
                  {settingsData.currency.items.map((currencyObj: any, currencyIndex: number) => (
                    <option key={'currency-' + currencyIndex}>{currencyObj.title}</option>
                  ))}
                </Form.Control>
              </Col>
            </Form.Group>
